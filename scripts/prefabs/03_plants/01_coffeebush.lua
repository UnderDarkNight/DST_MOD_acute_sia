...
local function dig_up_common(inst, worker, numberries)
    if inst.components.pickable ~= nil and inst.components.lootdropper ~= nil then
        ...
        if inst.components.pickable:IsBarren() then
			...
			inst.components.lootdropper:SpawnLootPrefab("acute_sia_plant_dug_coffeebush")
        end
    end
    inst:Remove()
end
...
local function fn()
    ...
    minimap:SetIcon("acute_sia_plant_coffeebush.tex")
    ...
    inst.components.pickable:SetUp("acute_sia_food_coffeebeans", TUNING.BERRY_REGROW_TIME)
    ...
    inst:ListenForEvent("_OnPlanted", function(inst,pt)
        inst.Transform:SetPosition(pt.x,0,pt.z)
        inst.components.pickable:OnTransplant()
    end)
...
return Prefab("acute_sia_plant_coffeebush", fn, assets)